...
from app.core.config import MODEL_DIR, HF_BASE_URL
from app.schemas.metadata import MetadataSchema
...
# ------------------------------------------------------------------
# Crop → TFLite model mapping
# ------------------------------------------------------------------
MODEL_FILES = {
    "apple": "best_apple_model.tflite",
    "grapes": "best_grapes_model.tflite",
    "tomato": "best_tomato_cnn_model.tflite",
    "corn": "corn_best_model.tflite",
    "potato": "potato_best_model.tflite",
}

# ------------------------------------------------------------------
# Interpreter cache (VERY IMPORTANT)
# ------------------------------------------------------------------
_INTERPRETERS: Dict[str, Dict] = {}

# ------------------------------------------------------------------
# Lazy loader (core function)
# ------------------------------------------------------------------
def get_model(crop: str) -> Dict:
    """
    Returns:
    {
    ...
    if crop in _INTERPRETERS:
        return _INTERPRETERS[crop]

    if crop not in MODEL_FILES:
        raise FileNotFoundError(
            f"No TFLite model mapping found for crop '{crop}'"
        )

    logger.info(f"Initializing TFLite model for crop '{crop}'")
    ...
    _INTERPRETERS[crop] = {
        ...
        "classes": metadata.root[crop]
    }

    logger.info(
        f"✅ TFLite model ready for '{crop}' | "
        f"Classes: {len(metadata.root[crop])}"
    )
